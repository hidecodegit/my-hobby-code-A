...
これは V字モデル SWE.2 の正式成果物です。
システムアーキテクチャをソフトウェアコンポーネントに分割し、各モジュールの責務とインターフェースを定義したものです。
...
| 日付 | 変更者 | 変更内容 |
|---|---|---|
| 2025-12-08 | Hideo & Gemini | SYS.1/SYS.2に合わせ、ドキュメントフォーマットを統一。目的セクションを削除し、概要説明に統合。 |
| 2025-12-07 | Hideo, Gemini | コンポーネント図をリスト形式に変更し、可読性を向上。Grokとの議論を反映し、設計トレードオフのセクションを追加。 |

### 用語集（本ドキュメント固有）
- **ソフトウェアコンポーネント**: `SensorCopier.py` を構成する主要な論理モジュール。
- **インターフェース**: モジュール間の呼び出し規約やデータ形式の定義。
...
### 3. トレーサビリティ（SYS.2 → SWE.2）

本表は、`SensorCopier.py` の責務範囲である要件のみを記載しています。
そのため、Macでの可視化（REQ-03, REQ-03.1）やCI/CD（REQ-04）、将来拡張のアーカイブ機能（REQ-04.1）は本ドキュメントの対象外としています。

| 要件ID | SYS.2 論理コンポーネント | SWE.2 ソフトウェアモジュール |
|---|---|
| REQ-01 | AHT25センサー読み取り | `SensorReader` |
| REQ-01.1 | 柔軟なタイムスタンプパース | `DataProcessor` |
| REQ-02 | rclone copy同期 | `Uploader` |
| REQ-02.1 | 月次JSON自動切替 + JST統一 | `DataWriter`, `SyncManager` |
| REQ-02.2 | Slackアラート（将来拡張） | `Alerter` |

### 4. 設計決定とトレードオフ（Design Decisions & Trade-offs）
| 決定事項 | 選択した理由 | 将来の対応予定 |
|---|---|---|
