...
|:------------------|:---------|:---------------------------------------------------------------------------------------------|
|**v6.0.0 (予定)**    |2025-12-XX|**Family-Friendly版**: latest_temp_humid.txt を月次ファイルから直近最大32行（約8時間分）抽出方式に変更。ユーザビリティ向上とデータ整合性強化。|
...
|**REQ-05**  |FR  |latest_temp_humid.txt に直近約8時間分の複数行データを保持し、即時参照可能とする|最大32行保持、最新行がファイル末尾、月次ファイルと完全整合|High    |3.3.1, 3.3.5, 4.1, 5.2|[ ] In Progress|
|**REQ-05.1**|NFR |latestファイルの更新はアトミックかつ月次ファイルと常に整合                   |更新成功率 100%、不整合ゼロ              |High    |3.3.5, 5.2            |[ ] In Progress|
...
- **latest複数行化の採用理由とトレードオフ**:
  - 採用案：月次ファイル依存の抽出方式。
  - 利点：再起動耐性100%維持、データ不整合リスクゼロ、実装シンプル、ユーザビリティ向上。
...
- **File Name**: `sensor_copier_v6_YYYYMMDD.py`（v6.0.0以降。リポジトリの最新を参照）
- **Language**: Python 3.x
...
- **追加予定テストケース（v6.0.0）**:
  - `update_latest_file(monthly_path, latest_path, max_lines=32)` の動作検証。
    - 月次ファイル10行 → latestに10行。
    - 月次ファイル50行 → latestに最新32行。
    - 月次ファイル不存在 → latest削除または空ファイル。
    - 月次ファイル0行（空） → latest空ファイル（警告ログ出力）。
...
> 本プロジェクトは、教訓駆動設計に加え、エンドユーザーの実利用体験を迅速に反映する柔軟性を獲得した。  
> 月次ファイルが唯一の真実源（Single Source of Truth）としてさらに強固になり、再起動耐性と家族満足度を両立するシステムへと進化した。
