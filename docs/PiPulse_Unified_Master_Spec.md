...
|:-----------|:-------|:------------|
| 2025-12-30 | Hideo, Grok, Gemini | **v1.3.3**: v5.2.0対策（CI環境分離設計）の教訓を反映。詳細は0.4を参照。 |
...
| 1.3 | v5.1.0までのリリース履歴だった。 | v5.2.0のリリース履歴を追加。 | 最新のコードリリースと同期するため。 |
| 3.3.7 | (セクションなし) | 「環境分離設計」を新設。 | CI/CDの安定性向上という重要な設計変更を明記するため。 |
| 7.2 | CI失敗の教訓が記載されていなかった。 | 「本番専用処理のCI汚染」対策の教訓への参照を追加。 | CIパイプラインの安定化に関する重要な学びを記録するため。 |
| 8.7 | (セクションなし) | 「新教訓レポート（CI汚染対策）」を新設。 | 7.2に記載するには長すぎる詳細な経緯と教訓を付録として保持するため。 |
...
|:---|:---|:---|
| **v5.2.0.20251230**|2025-12-30|**CI完全対応版**: 本番専用処理（ロギング・ディレクトリ作成）をCI環境でスキップするガードを実装。モジュールインポート時の副作用を排除し、ユニットテストの真の実行可能性を確保。|
...
- **目的**: 本番環境（RasPi）とテスト環境（GitHub Actions）の違いをコードレベルで吸収し、モジュールインポート時の失敗を根絶。
- **実装**:
  ```python
  ...
  ```
- **効果**: CIではファイルシステム操作を一切行わず、コンソールログのみ。テスト実行速度・安定性が飛躍的に向上。
- **教訓**: 「本番で必要な処理 = テストでも必要」とは限らない。**環境特有の副作用は明示的にガードすべし**。
...
### 4.1 実装仕様
- **File Name**: `sensor_copier_v5_20251230.py`（最新版はリポジトリの`sensor_copier_v5_YYYYMMDD.py`を参照）
- **Language**: Python 3.x
...
- **新教訓 (2025-12-30)**: 「本番専用処理のCI汚染」対策。
  - 本番特有のファイルシステム操作をインポート時に無条件実行していたため、CIでモジュールインポート失敗が頻発。
  - → IS_CIガードで環境分離を実現。詳細は **8.7** を参照。
...
### 8.7 新教訓レポート（CI汚染対策）
- **原因「これまでうっかりしていた」**:
  - 本番環境（RasPi）専用の処理（ログディレクトリ作成、RotatingFileHandlerの初期化、永続ディレクトリ確保）をモジュールインポート時に無条件実行していた。
  - CI環境（ubuntu-latest）では `/home/hideo_81_g/` が存在せず、PermissionError / FileNotFoundError でインポート自体が失敗 → ユニットテストが1つも走らない最悪の状態に。
- **結果「またびっくりした」**:
  - テストコードは完璧なのに、CIが赤くなる繰り返し。原因特定に時間をロス。
  - 「テストが通れば安心」と思い込んでいたが、**モジュールインポートの段階で落ちる**のは真のCI失敗。
- **対策「これからはしっかり環境分離する」**:
  - 環境変数 `GITHUB_ACTIONS` を用いて `IS_CI` フラグを導入。
  - 本番専用処理（ログファイルハンドラ、ディレクトリmkdir）を `if not IS_CI:` で完全にガード。
  - CIでは `StreamHandler`（コンソール出力）のみ使用 → テスト実行に一切影響なし。
- **深い教訓**:
  - **「本番環境特有の副作用は、インポート時に実行してはならない」**。
  - CI/CDの本当の価値は「テストが通る」だけでなく、「**本番コードがテスト環境で安全にインポート・実行可能**」であること。
  - この進化により、**v5.2.0 は「環境非依存のクリーンなモジュール」**へと昇華した。
