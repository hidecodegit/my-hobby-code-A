...
- **タイムゾーン方針**:
  ...
  - **移行注意**: UTCで記録されたJSONデータとの互換性問題が発生したため、タイムゾーン移行時は既存データとの整合性チェックが必須です（詳細: SYS.2 system-design.mdの事例セクション）。このJST統一方針と、後述のcronログ監視は、[REQ-02]の処理時間<1min目標の達成に貢献します。
- **潜在リスクと対策**:
  ...
  - [x] **運用ログ**: 全体同期の実行理由（初回、時間経過、ファイル破損など）をログに出力し、運用の透明性を確保。`software-specs.md`で定義されたcronログ監視を併用。
