...
# --- 設定 ---
__version__ = "4.0.0"  # 再起動時のデータ復元機能(DataRestorer)を追加した版
...
log_file_path = os.path.join(LOG_DIR, "sensor_copier_v4.log")
log_handlers = [
...
def restore_ram_from_persistent():
    """DataRestorer: RAM上の月次ファイルが消失している場合（再起動後など）、永続領域から復元する"""
    # 1. 月次ファイルの復元
    monthly_path_ram = get_monthly_filepath(RAM_DATA_DIR)
    monthly_path_persistent = get_monthly_filepath(PERSISTENT_DATA_DIR)

    if not os.path.exists(monthly_path_ram) and os.path.exists(monthly_path_persistent):
